...
router = APIRouter(prefix="/preparations/{preparation_id}/commentaires", tags=["commentaires"])
...
def list_commentaires(
    ...
    _get_preparation_or_404(preparation_id, db)
    commentaires = (
        db.query(Commentaire)
        .filter(Commentaire.brouillon_id == preparation_id)
...
def add_commentaire(
    ...
    if body.parent_id:
        parent = db.query(Commentaire).filter(
            ...
            Commentaire.brouillon_id == preparation_id,
        ).first()
        if not parent:
...
def resoudre_commentaire(
    ...
    preparation = db.query(Preparation).filter(Preparation.id == c.brouillon_id).first()
    is_auteur = preparation and preparation.auteur_id == current_user.id
    is_resp = current_user.role in (RoleEnum.responsable, RoleEnum.admin)
    if not is_auteur and not is_resp:
        raise HTTPException(
            status_code=403,
            detail="Seul l'auteur de la préparation ou un responsable peut résoudre un commentaire",
        )
    c.resolu = True
    db.commit()
    db.refresh(c)
